...
const encodeToRot13 = (str) => {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    let coded = charCode;
    if ((charCode > 64 && charCode < 78) || (charCode > 96 && charCode < 110)) coded += 13;
    if ((charCode > 77 && charCode < 91) || (charCode > 109 && charCode < 123)) coded -= 13;
    encoded += String.fromCharCode(coded);
  }
  return encoded;
};
...
const getCardId = (value) => {
  const figure = value.slice(0, -1);
  const color = value.charAt(value.length - 1);
  const template = {
    '♣': 0,
    '♦': 13,
    '♥': 26,
  ...
  const id = (+figure - 1 || template[figure]) + template[color];
  return id;
};
